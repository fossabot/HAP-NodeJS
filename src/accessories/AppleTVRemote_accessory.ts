...
/*
    This example plugin exposes an simply http api to interact with the remote and play around.
    The following routes are supported. The http runs on port 8080 as default.
    This example should not be used except for testing as the http server is unsecured.

    /press?button=<buttonId>&time=<timeInMS>  - presses a given button for a give time. Time is optional and defaults to 200
